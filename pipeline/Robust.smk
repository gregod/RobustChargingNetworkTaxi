...
rule run_robust_full_strat:
    ...
    output:
        stdout=OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{INT_NUM_SITES}{SUFFIX_DASH}/{NUM_VEHICLES}/full_opt_log_quorum:{QUORUM_ACCEPT}",
    shell:
...
rule generate_seed_median_robust_opt_log_file_name:
    ...
    input:
        ...
        optLogs=expand(OUTPUT_PREFIX + "/opt/{SEED}/group_{TYPE_GROUP}/{{BATTERY}}/tol{{TOLERANCE}}/{{NUM_SITES}}/{{NUM_VEHICLES}}/opt_log",SEED=rob_seeds_strat, TYPE_GROUP=rob_groups_strat),
    output:
         OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{NUM_SITES}/{NUM_VEHICLES}/median_seed_{is_leveled}opt_log_filename_tol{TOLERANCE}"
    shell:
...
rule generate_seed_lowest_robust_opt_log_file_name:
    ...
    input:
        ...
        optLogs=expand(OUTPUT_PREFIX + "/opt/{SEED}/group_{TYPE_GROUP}/{{BATTERY}}/tol{{TOLERANCE}}/{{NUM_SITES}}/{{NUM_VEHICLES}}/{{is_leveled}}opt_log",SEED=rob_seeds_strat, TYPE_GROUP=rob_groups_strat),
    output:
         OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{NUM_SITES}/{NUM_VEHICLES}/lowest_seed_{is_leveled}opt_log_filename_tol{TOLERANCE}"
    shell:
        "python {input.scriptE} {input.optLogs} > {output}"
...
rule cp_seed_robust_opt_log:
    input:
         OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{NUM_SITES}/{NUM_VEHICLES}/{SEED_TYPE}_seed_{is_leveled}opt_log_filename_tol{TOLERANCE}"
    output:
        OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{NUM_SITES}/{NUM_VEHICLES}/{SEED_TYPE}_{is_leveled}opt_log_tol{TOLERANCE}"
    shell:
        "cp $(cat {input}) {output}"
...
rule run_robust2_srat:
    ...
    input:
        sites=OUTPUT_PREFIX + "/preprocessed/{INT_NUM_SITES}{SUFFIX_DASH}.sites.csv",
        seed_vehicle_opt_log=OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{INT_NUM_SITES}{SUFFIX_DASH}/{NUM_VEHICLES}/{SEED_TYPE}_seed_opt_log_filename_tol00",
        vehicles=expand(OUTPUT_PREFIX + "/preprocessed/{SEED}/group_{TYPE_GROUP}/{{INT_NUM_SITES}}/{{NUM_VEHICLES}}/{{BATTERY}}.final.vehicles.csv.gz", SEED=rob_seeds_strat, TYPE_GROUP=rob_groups_strat),
        trips=expand(OUTPUT_PREFIX + "/preprocessed/{SEED}/group_{TYPE_GROUP}/{{INT_NUM_SITES}}/{{NUM_VEHICLES}}/{{BATTERY}}.final.trips.csv.gz", SEED=rob_seeds_strat, TYPE_GROUP=rob_groups_strat),
        battery=OUTPUT_PREFIX +"/preprocessed/{BATTERY}.toml",
    ...
    output:
        stdout=OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{INT_NUM_SITES}{SUFFIX_DASH}/{NUM_VEHICLES}/{SEED_TYPE}_opt_log_quorum:{QUORUM_ACCEPT}_activate:{MAX_ACTIVATE}_benevolent:{BENEVOLENT}_iis:{IIS}",
    shell:
...
rule extract_site_csv_robust_strat:
    ...
    input:
         ...
         optlog=OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{NUM_SITES}/{NUM_VEHICLES}/{PREFIX}_opt_log_{SUFFIX}"
    output:
          OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{NUM_SITES}/{NUM_VEHICLES}/{PREFIX}_active_sites_{SUFFIX}",
    conda:
         "environment.yaml"
    shell:
...
rule check_cross_feasibility_of_robust_strat:
    ...
    resources:
             runtime=1800
    input:
         active_sites=OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{INT_NUM_SITES}{SUFFIX_DASH}/{NUM_VEHICLES}/{PREFIX}_active_sites_{SUFFIX}",
         vehicles=expand(OUTPUT_PREFIX + "/preprocessed/{SEED}/group_{TYPE_GROUP}/{{INT_NUM_SITES}}/{{NUM_VEHICLES}}/{{BATTERY}}.final.vehicles.csv.gz", SEED=rob_seeds_strat, TYPE_GROUP=rob_groups_strat),
         trips=expand(OUTPUT_PREFIX + "/preprocessed/{SEED}/group_{TYPE_GROUP}/{{INT_NUM_SITES}}/{{NUM_VEHICLES}}/{{BATTERY}}.final.trips.csv.gz", SEED=rob_seeds_strat, TYPE_GROUP=rob_groups_strat),
         battery=OUTPUT_PREFIX +"/preprocessed/{BATTERY}.toml",
         binary=OUTPUT_PREFIX + "/binaries/check_cross_feasibility"
    output:
          OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{INT_NUM_SITES}{SUFFIX_DASH}/{NUM_VEHICLES}/{PREFIX}_cross_feasibility_{SUFFIX}"
    shell:
...
rule check_validation_cross_feasibility_of_robust_strat:
    ...
    input:
         ...
         binary=OUTPUT_PREFIX + "/binaries/check_cross_feasibility"
    output:
          OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{INT_NUM_SITES}{SUFFIX_DASH}/{NUM_VEHICLES}/{PREFIX}_validation_cross_feasibility_{SUFFIX}"
    shell:
         "{input.binary} --percent_infeasible_allowed 0.00  --sites {input.active_sites} --battery {input.battery}  --trips {input.trips} --vehicles {input.vehicles} | grep -v '^Academic license' > {output}"
...
rule evaluate_cross_feasibility_of_robust_strat:
    ...
    resources:
             runtime=1800
    input:
        cross_file=OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{NUM_SITES}/{NUM_VEHICLES}/{PREFIX}_cross_feasibility_{SUFFIX}",
        script="preprocessing/evaluate_cross_result.py"
    output:
        cross_file=OUTPUT_PREFIX + "/opt/robust/{BATTERY}/{NUM_SITES}/{NUM_VEHICLES}/{PREFIX}_percent_feasible_{SUFFIX}"
    shell:
